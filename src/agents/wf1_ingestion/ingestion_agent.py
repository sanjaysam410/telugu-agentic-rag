...
class IngestionAgent(BaseAgent):
    ...
    # TOGGLE: Set to True to enable Qdrant vector indexing.
    # Default OFF — stories go to MongoDB only for manual review first.
    # Turn ON after batch quality review to push verified stories to Qdrant.
    ENABLE_QDRANT_INDEXING = False

    def run(self, state: IngestionState) -> IngestionState:
        """
        Executes the ingestion pipeline.
        When ENABLE_QDRANT_INDEXING is False: MongoDB backup only.
        When True: MongoDB + Embedding + Qdrant upsert.
        """
        title = state.metadata.get('title', 'Untitled')
        self.log_step("Start", f"Ingesting story: {title}")

        try:
            # 1. MongoDB Backup (ALWAYS runs — cold storage)
            if self.mongo_client:
                self._store_in_mongodb(state)

            # 2 & 3. Embedding + Qdrant (ONLY if toggle is ON)
            if self.ENABLE_QDRANT_INDEXING:
                ...
                self.log_step("Qdrant", "SKIPPED — ENABLE_QDRANT_INDEXING is OFF. MongoDB-only mode.")
